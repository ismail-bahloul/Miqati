...
function installGlobals() {
  ...
  // Never reach the network from a unit test: report a newer release so the
  // update wiring can be asserted deterministically.
  globalThis.fetch = async () => ({
    ok: true,
    json: async () => ({ tag_name: "v0.2.2", html_url: "https://example.test/v0.2.2" }),
  });
...
test("a newer release is announced without downloading anything", async () => {
